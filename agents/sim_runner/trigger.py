...
def run_forward(t0: float, horizon_min: float = 120.0) -> Path:
    ...
    _log.info(f"[Forward Sim] scenario_id={scenario_id}  t0={t0}  horizon={horizon_min}min")
    ...
    _log.info(f"[Forward Sim] T0 스냅샷: WIP {wip_n}개, Tool {tool_n}개")

    # 3. VALIDATED 승격
    _promote_to_validated(scenario_id)

    # 4. run_sim_forward_once.py 호출
    _log.info("[Forward Sim] 시뮬레이션 실행 중...")
